...
class Reservation {
  ...
  static async create(data) {
    const { MemberId, FacilityId, ReservationDate, ReservationStartTime, 
            ReservationEndTime, ReservationStatus } = data;
    
    const [result] = await pool.query(
      `INSERT INTO Reservation (MemberId, FacilityId, ReservationDate, 
        ReservationStartTime, ReservationEndTime, ReservationStatus) 
       VALUES (?, ?, ?, ?, ?, ?)`,
      [MemberId, FacilityId, ReservationDate, ReservationStartTime, 
       ReservationEndTime, ReservationStatus || 'Pending']
    );
  ...
  static async update(id, data) {
    ...
    await pool.query(
      `UPDATE Reservation SET MemberId = ?, FacilityId = ?, ReservationDate = ?, 
       ReservationStartTime = ?, ReservationEndTime = ?, ReservationStatus = ? 
       WHERE ReservationId = ?`,
      [MemberId, FacilityId, ReservationDate, ReservationStartTime, 
       ReservationEndTime, ReservationStatus, id]
    );
  }
